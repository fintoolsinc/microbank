...
async function saveForm(clas){
    ...
    loadOption(clas+".search",'Options')
}
//-------------------------------------------------------//
//LEVEL 2
...
async function loadCombos(clas){
    let entity = await getCollectionPair('entity','id','desc');
                await fillSelect(entity,'option.entity.id');
    let component = await getCollectionPair('component','id','desc');
                  await fillSelect(component,'option.component.id');
    let container = await getCollectionPair('container','id','desc');
                  await fillSelect(container,'option.container.id');
}
//------------------------------------------------------//
//Level 3
